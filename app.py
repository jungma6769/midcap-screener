...
def screen():
    ...
    target = float(request.args.get("target", sc.M_TARGET))
    sigma = float(request.args.get("sigma", sc.SIGMA))
    max_dte = float(request.args.get("max_dte", sc.MAX_DTE))
    min_cash_conversion = float(request.args.get("min_cash_conversion", sc.MIN_CASH_CONVERSION))
    ...
    overlay = df.apply(
        lambda row: sc.formula6_overlay_score(
            row["market_cap_b"], row["debt_to_equity"], row["op_margin"], row["fcf_b"],
            target=target, sigma=sigma, max_dte=max_dte, min_cash_conversion=min_cash_conversion
    ...
    return jsonify({
        "params": {"target": target, "sigma": sigma, "max_dte": max_dte, "min_cash_conversion": min_cash_conversion},
        "count": len(df),
